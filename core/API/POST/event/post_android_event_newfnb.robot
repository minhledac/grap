...
Resource          set_android_event_payload.robot

*** Keyword ***
Post Android Event Sync Create Order And Add An Item
    ...
    ${primary_event}    ${aggregate_id}    Get android data payload create order    ${table_id}    ${pricebook_id}    ${dining_option}    ${branch_id}    ${user_id}    ${retailer_id}
    #---------
    ${secondary_event}    ${list_order_item_id}    Get android data payload of add order item    ${aggregate_id}    ${list_pr_id}    ${list_pr_price}    ${branch_id}    ${user_id}    ${retailer_id}    ${is_other_pr}
    ...
    Event Android Call From Template    ${template_input_dict}    ${branch_id}
    Return From Keyword    ${aggregate_id}    ${list_order_item_id}

Post Android Event Sync Add Order Item
...
Post Android Event Sync Change Pricebook
...
Post Android Event Sync Change Price Item
...
Post Android Event Sync Complete Order
...
Post Android Event Sync Change Customer
...
Post Android Event Sync Change Sale Channel
...
Post Android Event Sync Change Order Note
...
Post Android Event Sync Add Payment
...
Post Android Event Sync Remove And Add Payment
...
Post Android Event Sync Change Over Payment Type
...
Post Android Event Sync Change Order Type - Other Table
...
Post Android Event Sync Change Order Type - Delivery
...
Post Android Event Sync Change Delivery Info
    [Arguments]     ${aggregate_id}     ${ward}=${EMPTY}     ${status}=3    ${receiver}=${EMPTY}    ${contact_number}=${EMPTY}   ${address}=${EMPTY}    ${location_id}=${EMPTY}
    ...    ${delivery_code}=${EMPTY}   ${price}=${EMPTY}    ${delvery_date}=${EMPTY}    ${delivery_by}=${EMPTY}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
...
#hai
Post Android Event Sync Notified Kitchen
...
Post Android Event Sync Change Note And Toppings Item
...
Post Android Event Sync Change SoldBy
...
Post Android Event Sync Remove Order Item
...
Post Android Event Sync Remove Order Item And Notify Kitchen
...
Post Android Event Sync Cancel Order
...
Post Android Event Sync Applied Surcharge
    [Documentation]
...
Post Android Event Sync Change Item Priority
...
Post Android Event Sync Process Item
...
Post Android Event Sync Delivery Item
    [Arguments]    ${aggregate_id}    ${list_order_item_id}    ${list_quantity}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
...
Post Android Event Sync Adjust Item Quantity
...
Post Android Event Discount Order Event
...
Post Android Event Discount Item Order Event
...
Post Android Event Change Purchase Date Event
    [Arguments]    ${aggregate_id}    ${purchaseDate}=null    ${id_branch}=${BRANCH_ID}    ${id_user}=${USER_ID}    ${id_retailer}=${RETAILER_ID}
    ${result_data}    Get android data payload change purchase date    ${aggregate_id}    ${purchaseDate}
    ${template_input_dict}    Create Dictionary    primary_event=${result_data}
    Event Call From Template    ${template_input_dict}    ${id_branch}

#===============================================================================
# ---------- loan.nt: Chỉ sử dụng keyword này cho phần prepare-data xóa đơn hàng ở MHTN
Post Android Event Sync Cancel Order for prepare-data
    [Arguments]    ${aggregate_id}    ${cancel_reason_id}=0    ${reason_detail}=Khác    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${result_data}    Get android data payload cancel order for prepare-data    ${aggregate_id}    ${cancel_reason_id}    ${reason_detail}    ${branch_id}    ${user_id}    ${retailer_id}
    ${template_input_dict}    Create Dictionary    primary_event=${result_data}
    Event Call From Template    ${template_input_dict}    ${branch_id}

Get android data payload cancel order for prepare-data
    [Arguments]    ${aggregate_id}    ${cancel_reason_id}=0    ${reason_detail}=Khác    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ...
    ${payload_data}    KV Set Android Data For Input Dict Events for prepare-data    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

KV Set Android Data For Input Dict Events for prepare-data
