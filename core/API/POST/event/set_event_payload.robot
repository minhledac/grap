...
Get data payload create order
    ...
    ${order_code}     Set Variable   ${PrefixCode}-${Version_prefix}
    Set Suite Variable    ${Prefix_ver}    ${Version_prefix}
    # Gen ra chuỗi 11 số
    # ${order_code}   Generate Random String    11    [NUMBERS]
...
Get data payload of add order item
    ...
    ${template_str}    Get File    ${template_event_dir}/add_order_item_event.txt
    FOR    ${pr_id}    ${pr_price}    ${key_ver}    IN ZIP    ${list_pr_id}    ${list_pr_price}    ${list_key_ver}
        ...
        # Set string formulas
        ${str_formulas}    KV Set data formulas    ${pr_id}
        # Set giá trị của input dictionary
        ...
        ...    replica_id=${replica_id}         aggregate_id=${aggregate_id}    retailer_id=${retailer_id}        branch_id=${branch_id}   user_id=${user_id}    firebase_token=${firebase_token}
...
KV Set Quantity Type Of Product In Event
...
Get data payload of change price item
    ...
    ${list_data_event}   Create List
    ${template_str}    Get File    ${template_event_dir}/change_price_item_event.txt
...
Get data payload change pricebook
    ...
    ${template_str}    Get File    ${template_event_dir}/change_pricebook_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

#loan
Get data payload complete order
    [Arguments]    ${aggregate_id}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${input_dict}    Create Dictionary    aggregate_id=${aggregate_id}   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}
    ${template_str}    Get File    ${template_event_dir}/complete_order_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change customer
    ...
    ${template_str}    Get File    ${template_event_dir}/change_customer_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change sale channel
    ...
    ${template_str}    Get File    ${template_event_dir}/change_SaleChannel_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change order note
    [Arguments]    ${aggregate_id}    ${order_note}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${input_dict}    Create Dictionary    aggregate_id=${aggregate_id}   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}    firebase_token=${firebase_token}
    ...     order_note=${order_note}
    ${template_str}    Get File    ${template_event_dir}/change_order_note_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload add payment
    ...
    ${template_str}    Get File    ${template_event_dir}/add_payment_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload remove payment
    [Arguments]     ${aggregate_id}    ${remove_amount}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${input_dict}    Create Dictionary    aggregate_id=${aggregate_id}   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}    firebase_token=${firebase_token}
    ...     amount=${remove_amount}
    ${template_str}    Get File    ${EXECDIR}/api-data/test_event/remove_payment_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change over payment type
    ...
    ${template_str}    Get File    ${template_event_dir}/change_over_payment_type_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change table
    [Arguments]    ${aggregate_id}    ${table_id}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${input_dict}    Create Dictionary    aggregate_id=${aggregate_id}   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}    firebase_token=${firebase_token}
    ...     table_id=${table_id}
    ${template_str}    Get File    ${EXECDIR}/api-data/test_event/change_table_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change dining option
    [Arguments]    ${aggregate_id}    ${dining_option}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${input_dict}    Create Dictionary    aggregate_id=${aggregate_id}   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}    firebase_token=${firebase_token}
    ...     dining_option=${dining_option}
    ${template_str}    Get File    ${EXECDIR}/api-data/test_event/change_dining_option_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change delivery info
    ...
    ${template_str}    Get File    ${EXECDIR}/api-data/test_event/change_delivery_info_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}


#hai
Get data payload notify kitchen
    ...
    ${template_str}    Get File    ${template_event_dir}/notify_kitchen_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change note item
    ...
    ${template_str}    Get File    ${template_event_dir}/change_item_note_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change toppings item
    ...
    ...   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}
    ${template_str}    Get File    ${template_event_dir}/change_item_toppings_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change soldBy
    ...
    ${template_str}    Get File    ${template_event_dir}/change_soldBy_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload remove order item
    ...
    ${list_data_event}   Create List
    ${template_str}    Get File    ${template_event_dir}/remove_order_item_event.txt
...
Get data payload applied surcharge
    ...
    ${list_data_event}   Create List
    ${template_str}    Get File    ${template_event_dir}/applied_surcharge_event.txt
...
Get data payload change item priority
    ...
    ...   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}
    ${template_str}    Get File    ${template_event_dir}/change_item_priority_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload payment request
    [Arguments]    ${aggregate_id}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${input_dict}    Create Dictionary    aggregate_id=${aggregate_id}   retailer_id=${retailer_id}    branch_id=${branch_id}   user_id=${user_id}
    ${template_str}    Get File    ${template_event_dir}/payment_request_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload change number customer
    ...
    ${template_str}    Get File    ${template_event_dir}/change_number_customer_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

Get data payload process item
    ...
    ${list_key_ver}    KV Set List Key Version    ${list_order_item_id}
    ${list_data_event}   Create List
    ${template_str}    Get File    ${template_event_dir}/process_item_event.txt
...
Get data payload delivery item
    [Arguments]    ${aggregate_id}    ${list_order_item_id}    ${list_quantity}    ${branch_id}=${BRANCH_ID}    ${user_id}=${USER_ID}    ${retailer_id}=${RETAILER_ID}
    ${type_list_order_item_id}    Evaluate    type($list_order_item_id).__name__
    ${list_order_item_id}    Run Keyword If    '${type_list_order_item_id}'=='list'    Set Variable    ${list_order_item_id}    ELSE    Create List    ${list_order_item_id}
    ${list_quantity}         Run Keyword If    '${type_list_order_item_id}'=='list'    Set Variable    ${list_quantity}         ELSE    Create List    ${list_quantity}
    ${list_key_ver}    KV Set List Key Version    ${list_order_item_id}
    ${list_data_event}   Create List
    ${template_str}    Get File    ${template_event_dir}/delivery_item_event.txt
    FOR    ${order_item_id}    ${quantity}    ${key_ver}    IN ZIP    ${list_order_item_id}    ${list_quantity}    ${list_key_ver}
        ${cus_event_id_ver}    ${cus_track_ver}    ${cus_event_ver}    ${cus_local_time}    KV Set Common Key Version    ${key_ver}
        # Gen theo định dạng UUID
        ${notify_id}     Generate Random UUID
...
Get data payload of adjust item quantity
    ...
    ${list_key_ver}    KV Set List Key Version    ${list_pr_uuid}
    ${list_data_event}       Create List
    ${template_str}    Get File    ${template_event_dir}/adjust_item_quantity_event.txt
...
Get data payload of discount order event
...
Get data payload of discount item order event
...
Get data payload change purchase date
    ...
    ${template_str}    Get File    ${template_event_dir}/change_purchaseDate_event.txt
    ${payload_data}    KV Set Data For Input Dict Events    ${input_dict}     ver    ${template_str}
    Return From Keyword    ${payload_data}

#------------------------------------------------------- End Get data payload -----------------------------------------------------------------
KV Set Data Delivery Info
...
KV Set data cancel reason item
